...
namespace orc {
  ...
  public:
    ...
    void printRow(uint64_t rowId, char** tuple, unsigned int curColId) override;
  ...
  void writeString(std::string& file, const char *ptr) {
    ...
    /*cjq: writing char* value to tuple[curColId]*/
  void writeTuple(char ** tuple, unsigned int curColId, const char *ptr) {
     size_t len = strlen(ptr);
     tuple[curColId] = new char[len];
     memcpy (tuple[curColId], ptr, len+1);
  }
  ...
  void LongColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    if (hasNulls && !notNull[rowId]) {
      ...
      writeTuple(tuple, curColId, numBuffer);
    }
  }
  ...
  void DoubleColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    if (hasNulls && !notNull[rowId]) {
      tuple[curColId] = NULL;
    } else {
      char numBuffer[64];
  ...
  void Decimal64ColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    if (hasNulls && !notNull[rowId]) {
      tuple[curColId] = NULL;
    } else {
      /*writeString(buffer, toDecimalString(data[rowId], scale).c_str());*/
      writeTuple(tuple, curColId, toDecimalString(data[rowId], scale).c_str());
    }
  }
  ...
  Decimal128ColumnPrinter::Decimal128ColumnPrinter(std::string& buffer,
   ...
   void Decimal128ColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
     if (hasNulls && !notNull[rowId]) {
       tuple[curColId] = NULL;
     } else {
       /*writeString(buffer, data[rowId].toDecimalString(scale).c_str());*/
       writeTuple(tuple, curColId, data[rowId].toDecimalString(scale).c_str());
     }
   }
  ...
  void StringColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    if (hasNulls && !notNull[rowId]) {
      tuple[curColId] = NULL;
    } else {
      /* set mem directly rather than using writeTuple */
      tuple[curColId] = new char[length[rowId]+1];

      for(int64_t i=0; i < length[rowId]; ++i) {
        char ch = static_cast<char>(start[rowId][i]);
        tuple[curColId][i] = ch;
      }
	tuple[curColId][length[rowId]] = '\0';
    }
  }
  ...
  void ListColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    UNUSED(tuple);
    UNUSED(curColId);
    UNUSED(rowId);
    /*if (hasNulls && !notNull[rowId]) {
 ...
 *                                                                         }*/
  ...
  void MapColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    UNUSED(tuple);
    UNUSED(curColId);
    UNUSED(rowId);
    /*if (hasNulls && !notNull[rowId]) {
 ...
 *                                                                                                              */
  ...
  void UnionColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    UNUSED(tuple);
    UNUSED(curColId);
    UNUSED(rowId);
    /*if (hasNulls && !notNull[rowId]) {
 ...
 *                                                                             */
  ...
  void StructColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    UNUSED(tuple);
    UNUSED(curColId);
    if (hasNulls && !notNull[rowId]) {
    } else {
      for(unsigned int i=0; i < fieldPrinter.size(); ++i) {
        ...
        fieldPrinter[i]->printRow(rowId, tuple, i);
      }
    }
  }
  ...
  void DateColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    if (hasNulls && !notNull[rowId]) {
      /*writeString(buffer, "null");*/
      tuple[curColId] = NULL;
    } else {
      ...
      writeTuple(tuple, curColId, timeBuffer);
    }
  }
  ...
  void BooleanColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    if (hasNulls && !notNull[rowId]) {
      tuple[curColId] = NULL;
    } else {
      /*writeString(buffer, (data[rowId] ? "true" : "false"));*/
      writeTuple(tuple, curColId, (data[rowId] ? "true" : "false"));
    }
  }
  ...
  void BinaryColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
    if (hasNulls && !notNull[rowId]) {
      tuple[curColId] = NULL;
    } else {
      writeChar(buffer, '[');
  ...
  void TimestampColumnPrinter::printRow(uint64_t rowId, char** tuple, unsigned int curColId) {
